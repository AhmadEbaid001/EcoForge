...
function errorBox(error) {
  ...
  return `<div class="error-box">${escapeHtml(detail)}</div>`;
}
  ...
  async render(root, ctx) {
    ...
    await guard(root, async () => {
      ...
      root.innerHTML = `
        ...
        <section class="panel">
          <header><h3>Portfolio demand</h3><span class="muted">last 14 days of data time</span></header>
        ...
        <section class="panel">
          <header><h3>Alerts per day</h3><span class="muted">last 30 days of data time</span></header>
...
export const alerts = {
  title: 'Alerts',
  async render(root, ctx) {
    const state = { severity: '', onlyOpen: true };
    ...
    const draw = async () => {
      ...
      const rows = await api.anomalyFeed(params);
      ...
      body.innerHTML = `<table>
        <thead><tr>
          <th>Building</th><th>When (data time)</th><th>Observed</th><th>Expected</th>
          <th>z</th><th>Severity</th>${can(ctx.user, 'analyst') ? '<th></th>' : ''}
        </tr></thead>
        <tbody>${rows.map((row) => `
          <tr>
            <td>${escapeHtml(row.building_code)}</td>
            <td>${fmtDateTime(row.ts)}</td>
            <td>${compact(row.observed_kw)} kW</td>
            <td>${compact(row.expected_kw)} kW</td>
            <td>${row.robust_z === null ? '—' : row.robust_z.toFixed(1)}</td>
            <td><span class="sev ${escapeHtml(row.severity)}">${escapeHtml(row.severity)}</span></td>
            ${can(ctx.user, 'analyst')
              ? `<td><button class="ghost small" data-ack="${row.id}">Acknowledge</button></td>`
              : ''}
          </tr>`).join('')}
        </tbody></table>`;
    ...
    root.innerHTML = `
      <section class="panel">
        <header>
          <h3>Alert inbox</h3>
          <div class="controls">
            <select id="alert-sev" class="inline">
            ...
            ${can(ctx.user, 'analyst')
              ? '<button id="alert-ack-all" class="ghost small">Acknowledge all shown</button>' : ''}
          </div>
        </header>
    ...
    if (ackAll) {
      ackAll.addEventListener('click', async () => {
        if (!state.severity) {
          window.alert('Choose a severity first. Acknowledging everything at once is ' +
...
export const runs = {
  ...
  async render(root, ctx) {
    ...
    await guard(root, async () => {
      ...
      root.innerHTML = `
        <section class="panel">
          <header><h3>Stored allocations</h3><span class="muted">newest first</span></header>
          ${rows.length ? `<table>
            <thead><tr>
              <th>When</th><th>Budget</th><th>Objective</th><th>Solver</th>
              <th>Funded</th><th>Spent</th><th>Benefit</th><th>Inputs</th>
            </tr></thead>
            <tbody>${rows.map((r) => `<tr>
              <td>${fmtDateTime(r.created_at)}</td>
              <td>${compact(r.budget_egp)}</td>
    ...
    await guard(root, async () => {
      ...
      const draw = async () => {
        ...
        body.innerHTML = `
          ...
          ${report.break ? `<div class="error-box">First break: ${escapeHtml(report.break.reason)}
            at sequence ${report.break.seq}</div>` : ''}
...
export const admin = {
  ...
  async render(root, ctx) {
    ...
    await guard(root, async () => {
      ...
      root.innerHTML = `
        ${posture.warnings.length ? `<div class="warn-box">
        ...
        <section class="panel">
          ...
          ${posture.recent_failed_logins.length ? `<h4>Recent failed logins</h4>
            <table><tbody>${posture.recent_failed_logins.map((f) => `<tr>
        ...
        <section class="panel">
          <header><h3>Accounts</h3><button id="user-add" class="ghost small">Add user</button></header>
          <table>
            ...
            <tbody>${users.map((u) => `<tr>
              ...
              <td class="row-actions">
                <button class="ghost small" data-toggle="${escapeHtml(u.id)}"
                  data-active="${u.is_active}">${u.is_active ? 'Disable' : 'Enable'}</button>
                <button class="ghost small" data-reset="${escapeHtml(u.id)}">Reset password</button>
              </td>
            </tr>`).join('')}</tbody>
          </table>
        ...
        <section class="panel">
          <header><h3>Audit log</h3><span class="muted">most recent 50 events</span></header>
          <table>
      ...
      root.querySelectorAll('[data-reset]').forEach((button) => {
        button.addEventListener('click', async () => {
          const password = window.prompt(
            'New password for this account (at least 12 characters).\n' +
            'They will be required to change it at next login.');
          if (!password) return;
          await guard(root, async () => {
            await api.resetPassword(button.dataset.reset, password);
            await reload();
          });
        });
      });
      ...
      root.querySelector('#user-add').addEventListener('click', async () => {
        ...
        if (!password) return;
        const role = window.prompt('Role: viewer, analyst or admin', 'viewer');
        if (!role) return;
        await guard(root, async () => {
      ...
      root.innerHTML = `
        <section class="panel narrow">
          ...
          <form id="pw-form" class="form">
            ...
            <label>New password<input type="password" id="pw-new" minlength="12" required></label>
