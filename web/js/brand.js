...
/* Cropped to the G and its energy motif, for the 64px icon rail the sidebar
 * collapses to, where a 4.4:1 wordmark has nowhere to go.
 *
 * A viewBox, not a second drawing: the letters are one path, so narrowing the window
 * hides E, M and P without touching the artwork. The cut falls in the gap between G
 * and E, so nothing is clipped mid-letter. */
const MARK_VIEWBOX = '575 931 270 201';

/* `title` is what a screen reader announces. Both marks spell the name, so the pages
 * that use them drop their duplicate text heading rather than saying it twice. */
...
export function mark(title = 'GEMP') {
  return `<svg class="brand-mark-svg" viewBox="${MARK_VIEWBOX}" role="img"
    aria-label="${title}" focusable="false">${PATHS}</svg>`;
}
